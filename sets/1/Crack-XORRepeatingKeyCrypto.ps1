...
.DESCRIPTION
Crak-XORRepeatingKeyCrypto.ps1 uses Hamming Distance and Greatest
Common Denominator calculations to probablistically determine the size
of a repeating XOR key that may have been used for encrypting a string
or file.

Once the key size has been (probablistically) determined, this script
uses English language character frequencies to brute-force the key.
Once the key has been (probablistically) determined, the script will
use that key to decrypt the string or file.

The script works with base16 or base64 input, either from a file or
from a string passed via the command line.

The script will return an object some metadata about its analysis,
...
The script also returns the probable key
